...
function isCheckmate(king) {
    if (!isCheck(king)) return false;

    // Check all potential moves for the king
    const kingPos = findKing(king);
    for (let row = -1; row <= 1; row++) {
        for (let col = -1; col <= 1; col++) {
            const newRow = kingPos.row + row;
            const newCol = kingPos.col + col;
            if (newRow >= 0 && newRow < 8 && newCol >= 0 && newCol < 8) {
                if (isValidMove(king, kingPos, newRow, newCol) && !isCheckAfterMove(kingPos, { row: newRow, col: newCol })) {
                    return false;
                }
            }
        }
